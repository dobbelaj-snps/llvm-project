...
// Track the scopes used by !alias.scope.
// @llvm.experimental.noalias.scope.decl that declare a scope that is not used
// by a !alias.scope can be omitted.
class AliasScopeTracker {
  SmallPtrSet<const MDNode *, 8> UsedAliasScopes;
...
public:
    ...
    if (auto *MD = I->getMetadata(LLVMContext::MD_alias_scope))
      for (auto &MDOperand : cast<MDNode>(MD)->operands())
        if (auto *MDScope = dyn_cast<MDNode>(MDOperand))
          UsedAliasScopes.insert(MDScope);
  }
  ...
  bool isNoAliasScopeDeclDead(Instruction *Inst) {
    ...
    if (auto *MD = dyn_cast<MDNode>(MDOperand))
      return !UsedAliasScopes.contains(MD);
