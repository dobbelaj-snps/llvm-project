...
bool LoopRotate::rotateLoop(Loop *L, bool SimplifiedLatch) {
  ...
  do {
    ...
    if (!NoAliasDeclInstructions.empty() ||
        !NoAliasScopeDeclInstructions.empty()) {
      ...
      // or with D: llvm.noalias.decl,
      //         U: llvm.provenance.noalias depending on D
      ...
      // (1) -> : ... D' { D U1 U2 D'' }
      // (2) -> : ... D' U1' { D U2 D'' U1'' }
      // D: original llvm.experimental.noalias.scope.decl/llvm.noalias.decl
      ...
      // Clone the llvm.experimental.noalias.decl/llvm.noalias.decl again for
      // the NewHeader.
      ...
      for (Instruction *NAD : NoAliasDeclInstructions) {
        ...
        Instruction *NewNAD = NAD->clone();
        //@ FIXME: NewNAD->insertBefore(NewHeaderInsertionPoint);
      ...
      // OrigPreHeader'.
      {
        ...
        for (auto *NAD : NoAliasScopeDeclInstructions)
          NoAliasDeclScopes.push_back(NAD->getScopeList());
        for (auto *NAD : NoAliasDeclInstructions)
        ...
        // FIXME: originally we were updating only a part of the OrigPreHeader
        // FIXME: for now, for easyness, we update the complete OrigPreHeader.
        // FIXME: the comment below is not true any more !
        ...
        // large.
        LLVM_DEBUG(dbgs() << "  Updating OrigPreheader scopes\n");
        cloneAndAdaptNoAliasScopes(NoAliasDeclScopes, {OrigPreheader}, Context,
                                   "pre.rot");
