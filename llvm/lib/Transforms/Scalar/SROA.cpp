...
static llvm::Instruction *introduceNoAliasWhenCopyGuardIndicesAreCompatible(
  ...
  Type *TypeToLoad =
          PtrOp->getType()->isOpaquePointerTy() ?
              PtrOp->getType() : // FIXME: need to take addrspace into account ?
              PtrOp->getType()->getNonOpaquePointerElementType();
  ...
  if (IsLoadOfInteger) {
      ...
      NewPtr = IRB.CreatePointerBitCastOrAddrSpaceCast(
                  NewPtrOld, TypeToLoad->getPointerTo());
