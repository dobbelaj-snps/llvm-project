// RUN: %clang_cc1 -triple x86_64-apple-darwin -O2 -ffull-restrict -fno-experimental-new-pass-manager %s -emit-llvm -o - | FileCheck %s
// RUN: %clang_cc1 -triple x86_64-apple-darwin -O2 -ffull-restrict -fexperimental-new-pass-manager %s -emit-llvm -o - | FileCheck %s
...
int *test06(int *p, int n) {
...

  {
    ...
    // llvm.provenance.noalias rp (p -> (p || rp1))
    return rp;
  }
}
